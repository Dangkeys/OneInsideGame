...
public class UpdateLobbyUI : MonoBehaviour
{
    ...
    private bool IsValidConfiguration()
    {
        ...
        if (imposterCount < OneInside.Constants.Player.MIN_IMPOSTERS)
            return false;
        if (imposterCount > OneInside.Constants.Player.MAX_IMPOSTERS)
            return false;
    ...
    private async void UpdateLobby()
    {
        ...
        {
            UIManager.Instance.ShowMessage("Invalid lobby configuration!");
            return;
        }
        ...
        try
        {
            ...
            var updatedLobby = await LobbyManager.Instance.UpdateCurrentLobbyAsync(updateLobbyDto);

            if (updatedLobby == null)
            {
                UIManager.Instance.ShowMessage("Failed to update lobby!");
                return;
            }

            UIManager.Instance.ShowMessage("Lobby updated successfully!");
        }
        catch (System.Exception e)
        {
            UIManager.Instance.ShowMessage($"Error updating lobby: {e.Message}");
