...
public class StartHostUI : MonoBehaviour
{
    ...
    private async void HostMatch()
    {
        ...
        {
            OneInsideGameManager.Instance.UIManager.ShowMessage("Please enter a valid lobby name");
            return;
        }
        ...
        if (imposterCount < OneInside.Constants.Player.MIN_IMPOSTERS ||
            ...
            playerCount > OneInside.Constants.Player.MAX_PLAYERS)
        {
            OneInsideGameManager.Instance.UIManager.ShowMessage("Invalid player or imposter count");
            return;
        }
